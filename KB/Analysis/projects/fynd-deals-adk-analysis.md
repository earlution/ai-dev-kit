...
- ⚠️ Different KB structure (`knowledge/fynd_deals/Kanban/` vs `KB/PM_and_Portfolio/kanban/`)
...
- **Difference:** Uses `knowledge/fynd_deals/Kanban/` instead of `KB/PM_and_Portfolio/kanban/`
...
1. **Different KB Path**
   - **Issue:** Uses `knowledge/fynd_deals/Kanban/` instead of `KB/PM_and_Portfolio/kanban/`
...
**For This Project:**
1. **Consider Migration** - Evaluate migrating to ADK canonical structure (`KB/PM_and_Portfolio/kanban/`)
...
3. **Flexible Path Configuration**
   ...
   - Support `knowledge/` and `KB/` structures
...
- **Directory Structure:** `knowledge/fynd_deals/` (not `KB/`)
...
- **Root Path:** `knowledge/fynd_deals/` vs ADK `KB/`
...
1. **Incompatible KB Structure**
   - **Issue:** Uses `knowledge/` instead of `KB/`
...
1. **Support Multiple KB Paths**
   - Make ADK tools configurable for `knowledge/` and `KB/` paths
   - Don't hardcode `KB/` path
...
1. **Support Pre-ADK Projects**
   ...
   - **How:**
     ...
     - Support `knowledge/` and `KB/` paths
     - Don't hardcode paths
...
1. **Hardcoded Paths**
   - **Anti-Pattern:** Hardcoding `KB/` path in ADK tools
...
1. **Path Hardcoding**
   - **Issue:** ADK tools may hardcode `KB/` path
   - **How to Fix:** Make paths configurable, support `knowledge/` and `KB/`
...
1. **Support Multiple KB Paths**
   - **Current Approach:** May assume `KB/` path
   - **Better Approach:**
...
1. **Pre-ADK Projects Have Different Structures**
   ...
   - Different KB path (`knowledge/` vs `KB/`)
   - Custom implementations common
...
| File Organization | KB/PM_and_Portfolio/kanban/ | knowledge/fynd_deals/Kanban/ | ⚠️ DIVERGES |

**KB Structure Comparison:**

| Aspect | ADK Canonical | fynd.deals | Match? |
|--------|---------------|------------|--------|
| Root Path | KB/ | knowledge/fynd_deals/ | ❌ NO |
| Directory Organization | Architecture, Changelog, Documentation, Guides, PM_and_Portfolio | Kanban, debugging, specs, templates | ❌ NO |
