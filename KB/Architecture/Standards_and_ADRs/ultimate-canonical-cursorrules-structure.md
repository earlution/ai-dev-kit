...
- Generate detailed changelog: `KB/Changelog_and_Release_Notes/Changelog_Archive/CHANGELOG_v0.N.NNN.T+B.md`
...
Load config from `rw-config.yaml`:
- `kanban_root`: Path to Kanban root (e.g., `KB/PM_and_Portfolio/kanban`)
...
Documents in `KB/` directory must have lifecycle metadata in frontmatter:
